...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["category_id"], ["incident_categories.id"], ondelete="RESTRICT"
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "total_votes", sa.Integer(), nullable=False, server_default=sa.text("0")
        ),
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "student_id", "suggestion_id", name="unique_vote_per_student"
        ),
        if_not_exists=True,
    )
