...
s, b = req(
    "POST", "/auth/login", {"email": "estudiante@usb.ve", "password": "estudiante123"}
)
check(
    "Login estudiante",
    s,
    200,
    b,
    lambda b: ("access_token" in b, "access_token recibido"),
)
...
s, b = req(
    "POST",
    "/items/",
    {"name": "Item del estudiante", "description": "Test ownership"},
    est_token,
)
check(
    "Crear item",
    ...
    b,
    lambda b: (
        b.get("owner_id") is not None,
        f"owner_id={b.get('owner_id', '?')[:12]}...",
    ),
)
item_id = b.get("id", "")
...
check(
    ...
    200,
    b,
    lambda b: (isinstance(b, list) and len(b) > 0, f"{len(b)} items"),
)
...
check(
    "Tecnico DELETE item ajeno → 403",
    s,
    403,
    b,
    lambda b: (
        b.get("detail", {}).get("error_code") == "CROSS_ACCESS_DENIED",
        f"error_code={b.get('detail', {}).get('error_code')}",
    ),
)
...
check(
    "List con token revocado → 401",
    s,
    401,
    b,
    lambda b: (
        b.get("detail", {}).get("error_code") == "TOKEN_REVOKED",
        f"error_code={b.get('detail', {}).get('error_code')}",
    ),
)
...
check(
    "GET /me con token valido",
    s,
    200,
    b,
    lambda b: (
        b.get("user_id") is not None,
        f"user_id={b.get('user_id', '?')[:12]}...",
    ),
)
